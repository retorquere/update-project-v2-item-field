...
export class Interactor {
  ...
  async fetchProjectV2FieldByName(projectV2Id: string): Promise<ProjectV2Field> {
  ...
  async updateAllItemFields(projectV2Id: ProjectV2Id, field: ProjectV2Field): Promise<void> {
  ...
  async updateSingleItemField(projectV2Id: ProjectV2Id, field: ProjectV2Field, contentId: string): Promise<void> {
  ...
  async updateItemField(projectV2Id: ProjectV2Id, item: Item, field: ProjectV2Field): Promise<void> {
    // Check the skipUpdateScript
    if (this.inputs.skipUpdateScript) {
      const isSkip = await callAsyncFunction({ context, item }, this.inputs.skipUpdateScript)
    ...
    const value = this.inputs.fieldValue !== '' ? this.inputs.fieldValue : String(await callAsyncFunction({ context, item }, this.inputs.fieldValueScript))
...
function buildProjectV2FieldValue(field: ProjectV2Field, value: string): ProjectV2FieldValue {
